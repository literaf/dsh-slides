...
- 文件通过 `ctx.fs` 写入，不走 `node:fs`，所以沙箱文件系统后端会像管别的工具一样管住这次写入。也正因如此，**只有组合里存在文件系统提供者时才会注册这个工具**——没有的话工具和指令都不出现，而不是给模型一个必然失败的调用。
...
- **不导出 `.pptx`。** dsh 的文件系统能力只开放了文本写入，用别的方式写二进制会绕过管住所有工具的沙箱策略。目前请先打印成 PDF；等有了正规的二进制写入口子再补。
